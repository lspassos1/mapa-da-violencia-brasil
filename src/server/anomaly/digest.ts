...
import "server-only";
import { getElectoralAnomalies } from "@/server/anomaly/electoralCycle";
...
export interface DigestSignals {
  indicador: string;
  eleitorais: { uf: string; porte: string; efeito: number; efeitoRelativo: number }[];
...
// Coleta os sinais que merecem destaque: indícios eleitorais robustos (caem mais
// que os pares) + municípios do RJ com indício de controle territorial.
export function gatherSignals(): DigestSignals {
  const eleitorais = getElectoralAnomalies()
    .filter((u) => u.robusto && u.efeitoRelativo !== null && u.efeitoRelativo <= -0.05)
    .map((u) => ({ uf: u.uf, porte: u.porte, efeito: u.efeito as number, efeitoRelativo: u.efeitoRelativo as number }));
...
LENTE 1 — ciclo eleitoral (comparação por pares de mesmo porte, diff-in-diff; nunca ranking cru):
${linhasEle}
